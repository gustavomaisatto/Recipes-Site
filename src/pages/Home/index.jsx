...
export default function Home({ setRecipeSelect }) {
  ...
  useEffect(() => {
    async function fetchCategory() {
      await api.get(`/filter.php?c=${category}`).then((response) => {
  ...
  return (
    <Container>
      <RandomRecipe
        setRecipeSelect={setRecipeSelect}
        categorySelect={randomRecipe.idMeal}
        title={randomRecipe.strMeal}
        imageRandom={randomRecipe.strMealThumb}
      />
      <Categories
            ...
            return (
              ...
              <Link to={"/Recipes"} style={linkStyle}>
                <div
                  ...
                  onClick={() => setRecipeSelect(category.idMeal)}
                  key={category.idMeal}
                >
